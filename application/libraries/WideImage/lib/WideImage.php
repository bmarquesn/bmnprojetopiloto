 ...
 * @copyright 2007-2011
  ...
  **/
	...
	class LibWideImage
	{
		 ...
		 * Example:
		 * <code>
		 * 	LibWideImage::registerCustomMapper('WideImage_Mapper_TGA', 'image/tga', 'tga');
		 * </code>
		 * 
		 ...
		 * 
		 * This function analyzes the input and decides whether to use LibWideImage::loadFromHandle(),
		 * LibWideImage::loadFromFile(), LibWideImage::loadFromUpload() or LibWideImage::loadFromString(),
		 ...
		 * To load only a single image from an array field, use LibWideImage::loadFromUpload('img', $i), 
		 * where $i is the index of the image you want to load.
		 * 
		 * <code>
		 * $img = LibWideImage::load('http://url/image.png'); // image URL
		 * $img = LibWideImage::load('/path/to/image.png'); // local file path
		 * $img = LibWideImage::load('img'); // upload field name
		 * $img = LibWideImage::load(imagecreatetruecolor(10, 10)); // a GD resource
		 * $img = LibWideImage::load($image_data); // binary string containing image data
		 * </code>
		 * 
		...
		static function load($source)
		{
			...
			return call_user_func(array('LibWideImage', 'loadFrom' . $predictedSourceType), $source);
		}
		 ...
		 * $image = LibWideImage::loadFromHandle($handle);
		 * </code>
		 * 
	...
	require_once LibWideImage::path() . 'Exception.php';
	
	require_once LibWideImage::path() . 'Image.php';
	require_once LibWideImage::path() . 'TrueColorImage.php';
	require_once LibWideImage::path() . 'PaletteImage.php';
	
	require_once LibWideImage::path() . 'Coordinate.php';
	require_once LibWideImage::path() . 'Canvas.php';
	require_once LibWideImage::path() . 'MapperFactory.php';
	require_once LibWideImage::path() . 'OperationFactory.php';
	
	require_once LibWideImage::path() . 'Font/TTF.php';
	require_once LibWideImage::path() . 'Font/GDF.php';
	require_once LibWideImage::path() . 'Font/PS.php';
	...
	LibWideImage::checkGD();
	
	LibWideImage::registerCustomMapper('WideImage_Mapper_BMP', 'image/bmp', 'bmp');
	LibWideImage::registerCustomMapper('WideImage_Mapper_TGA', 'image/tga', 'tga');
