  ...
  **/
	...
	class WideImage_Operation_RoundCorners
	{
		...
		function execute($image, $radius, $color, $smoothness, $corners)
		{
			...
			$corner = LibWideImage::createTrueColorImage($radius * $sample_ratio, $radius * $sample_ratio);
			if ($color === null)
			{
				...
				if ($corners & LibWideImage::SIDE_TOP_LEFT || $corners & LibWideImage::SIDE_LEFT || $corners & LibWideImage::SIDE_TOP)
					$result = $result->applyMask($corner, -1, -1);
				
				$corner = $corner->rotate(90);
				if ($corners & LibWideImage::SIDE_TOP_RIGHT || $corners & LibWideImage::SIDE_TOP || $corners & LibWideImage::SIDE_RIGHT)
					$result = $result->applyMask($corner, $result->getWidth() - $corner->getWidth() + 1, -1, 100);
				
				$corner = $corner->rotate(90);
				if ($corners & LibWideImage::SIDE_BOTTOM_RIGHT || $corners & LibWideImage::SIDE_RIGHT || $corners & LibWideImage::SIDE_BOTTOM)
					$result = $result->applyMask($corner, $result->getWidth() - $corner->getWidth() + 1, $result->getHeight() - $corner->getHeight() + 1, 100);
				
				$corner = $corner->rotate(90);
				if ($corners & LibWideImage::SIDE_BOTTOM_LEFT || $corners & LibWideImage::SIDE_LEFT || $corners & LibWideImage::SIDE_BOTTOM)
